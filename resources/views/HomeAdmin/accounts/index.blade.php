    ...
    <table class="table">
        ...
        @foreach ($customers as $customer)
            <tr>
                ...
                <td>
                    <a href="{{ route('admin.accounts.edit', $customer['id']) }}" class="btn btn-warning">Edit</a>
                    <form action="{{ route('admin.accounts.destroy', $customer['id']) }}" method="POST" style="display:inline;">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="btn btn-danger" onclick="return confirm('Are you sure?')">Delete</button>
                    </form>
